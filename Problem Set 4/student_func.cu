...
void your_sort(unsigned int* const d_inputVals,
  ...
  const size_t numElems)
  {
    ...
    // Allocate device memory for the relative offsets
    int *d_relative_offsets;
    ...
    for (i = 0; i < LENGHT; i++) {
      ...
      // Calculate relative offsets for bits/digit that are equal to 0
      ...
      // Calculate relative offsets for bits/digit that are equal to 1
